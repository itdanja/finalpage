...
// * 전역 배열
let contents = [ {date : '2024-03-07' , content : '이젠학원 종강일' } ] // 여러개 일정객체를 저장하는 배열
...
function calPrint(){ 
	...
	let html = '';
		...
		for( let day = 1 ; day<=eDay ; day++ ){
			...
			html += `<div > 
						${ day } 
						${ contentPrint( `${year}-${ month < 10 ? '0'+month : month }-${day < 10 ? '0'+day : day}`) }	
					</div>`
...
function 일정등록(){
    ...
    contents.push( {date:date , content:content} );
